...
const readThirdPartyManifest = (directory: string): PluginManifest => {
  ...
  if (!existsSync(manifestPath)) throw new Error('插件目录中缺少 manifest.json。')
  const manifest = validatePluginManifest(JSON.parse(readFileSync(manifestPath, 'utf8')))
  if (manifest.source !== 'third-party') throw new Error('导入的插件必须声明为 third-party。')
  return manifest
}
...
const ensureNoSymbolicLinks = (directory: string): void => {
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (lstatSync(entryPath).isSymbolicLink()) throw new Error('插件包不能包含符号链接。')
...
export const installPluginFromDirectory = (sourceDirectory: string): InstalledPlugin => {
  if (!isAbsolute(sourceDirectory)) throw new Error('插件目录必须使用绝对路径。')
  const source = resolve(sourceDirectory)
  if (!existsSync(source) || !lstatSync(source).isDirectory()) throw new Error('插件目录不存在。')
  ...
  if (listInstalledPlugins().some((plugin) => plugin.manifest.id === manifest.id)) {
    throw new Error('同 ID 的插件已安装，请先卸载旧版本。')
  }
...
export const uninstallPlugin = (pluginId: string): void => {
  ...
  if (!plugin) throw new Error('只能卸载已安装的第三方插件。')
