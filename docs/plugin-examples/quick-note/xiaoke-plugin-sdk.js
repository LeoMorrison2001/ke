export class XiaokePlugin {
  constructor() { this.pending = new Map(); this.theme = 'light'; this.themeListeners = new Set(); window.addEventListener('message', (event) => this.handleMessage(event)) }
  onThemeChange(listener) { this.themeListeners.add(listener); listener(this.theme); return () => this.themeListeners.delete(listener) }
