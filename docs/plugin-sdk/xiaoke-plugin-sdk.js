export class XiaokePlugin {
  ...
  request(request) {
    ...
    window.parent.postMessage({ type: 'ke-plugin:request', requestId, request }, '*')
    return new Promise((resolve, reject) => this.pending.set(requestId, { resolve, reject }))
  }

  storage = {
    get: (key) => this.request({ operation: 'storage.get', key }),
    set: (key, value) => this.request({ operation: 'storage.set', key, value }),
    delete: (key) => this.request({ operation: 'storage.delete', key })
  }
  ...
  handleMessage(event) {
    ...
    if (message.type === 'ke-plugin:response') {
  ...
  async handleAgentInvocation(message) {
    try {
      ...
      window.parent.postMessage({ type: 'ke-plugin:agent-result', invocationId: message.invocationId, result: { status: 'failed', replyHint: error instanceof Error ? error.message : '插件 Agent 执行失败。' } }, '*')
    }
  }
}
